...
!   Observation points (fixed+moving)
   ...
   function build_nc_dimension_id_count_array(nc_dim_ids, ihisfile) result(counts)
      ...
      counts = [integer :: (get_dimid_len(ihisfile, dim_ids(i)), i=1, size(dim_ids))]
   ...
   subroutine write_station_netcdf_variable(output_variable_item, ihisfile, it_his)
      ...
      associate (nc_dim_ids => output_variable_item%output_config%nc_dim_ids)
         ...
         positions = [integer :: (i, i=1, size(counts))]
   ...
   subroutine unc_put_his_structure_static_vars(ncid)
      ...
      if (jaoldstr == 1) then
         ...
         indices = [integer :: (genstru2cgen(i), i=1, ngenstru)]
      ...
      if (network%sts%numGates > 0 .and. associated(network%sts%gateIndices) .and. associated(network%sts%struct)) then
         ...
         indices = [integer :: (gate2cgen(i), i=1, ngategen)]
